...
def parse_args():
    parser = argparse.ArgumentParser(description="Gemma-3-4B Inference Test")
    parser.add_argument("--model_name_or_path", type=str, default="google/gemma-3-4b-it",
...
def run_inference(prompt, model, tokenizer, logger, messages):
    logger.info(f"Executing prompt: {prompt[:50]}... (truncated)")
...
def main():
    ...
    for local_idx, seed in enumerate(target_seeds):
        ...
        if exp_type_idx == 0:
            ...
            output_path = f"subject_data/gemma3/4b/experiment_list_{subject_id}.csv"
            target_csv = bas_path
            is_bas = True
        elif exp_type_idx == 1:
        ...
        try:
            ...
            output_filename = f"results/gemma3/4b/result_{subject_id}.csv"
